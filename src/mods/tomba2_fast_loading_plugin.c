 ...
 * routes (GH TombaRecomp#5, ported to Tomba 2). The two are independent now; enabling both is
 * allowed and simply applies both.
 */
#define PKG "tomba2.enhancement.fast-loading"
...
static void tomba2_fast_loading_activate(void) {
...
PSX_MOD_CONSTRUCTOR(tomba2_register_fast_loading_plugin) {
    (void)psx_mod_register_activation_plugin(
        "tomba2.fast-loading", tomba2_fast_loading_activate);
}
